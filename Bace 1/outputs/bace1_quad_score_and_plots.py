...
# PATHS
# =======================
INPUT_CSV = r"C:\Users\poula\Desktop\quad redo\Bace 1\outputs\bace1_test_with_ad_risk.csv"
...
def pearson_r(x, y):
    x = x - x.mean()
    y = y - y.mean()
    return float((x*y).sum() / (np.sqrt((x*x).sum()) * np.sqrt((y*y).sum())))
...
def main():
    ...
    # QUAD score (equal weights, heuristic)
    ...
    # Save CSV
    out_csv = os.path.join(OUT_DIR, "bace1_test_with_QUAD.csv")
    ...
    plt.title(f"BACE1: |error| vs QUAD\nPearson r={pr:.3f}, Spearman ρ={sr:.3f}")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "A_bace1_error_vs_QUAD.png"), dpi=300)
    plt.close()
    ...
    plt.ylabel("|Prediction error|")
    plt.legend()
    plt.title("BACE1: Error vs individual risk components")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "B_bace1_error_vs_components.png"), dpi=300)
    plt.close()
    ...
    plt.xlabel("QUAD score")
    plt.ylabel("Count")
    plt.title("BACE1: QUAD score distribution")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "C_bace1_QUAD_distribution.png"), dpi=300)
    plt.close()

    print("✅ QUAD computed for BACE1")
