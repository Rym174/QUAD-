...
# PATHS
# =======================
INPUT_CSV = r"C:\Users\poula\Desktop\quad redo\Bace 1\outputs\bace1_rf_ensemble_uncertainty.csv"
...
def main():
    ...
    # Save augmented CSV
    out_csv = os.path.join(OUT_DIR, "bace1_test_with_error.csv")
    ...
    plt.title(f"BACE1: |error| vs uncertainty\nPearson r={pr:.3f}, Spearman ρ={sr:.3f}")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "A_bace1_error_vs_uncertainty.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"BACE1: log|error| vs uncertainty\nPearson r={pr_log:.3f}, Spearman ρ={sr_log:.3f}")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "B_bace1_logerror_vs_uncertainty.png"), dpi=300)
    plt.close()
    ...
    plt.title("BACE1: uncertainty vs predicted activity")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "C_bace1_uncertainty_vs_prediction.png"), dpi=300)
    plt.close()
    ...
    plt.ylabel("Count")
    plt.title("BACE1: distribution of RF uncertainty")
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, "D_bace1_uncertainty_distribution.png"), dpi=300)
    plt.close()

    print("✅ ALL BACE1 uncertainty plots generated")
