...
# PATHS (BACE1)
# =======================
# Full frozen split CSV (contains BOTH train+test + fingerprints)
SPLIT_CSV = r"C:\Users\poula\Desktop\quad redo\Bace 1\outputs\bace1_frozen_split_ecfp4_2048.csv"

# Test-set file that already has RF mean/uncertainty + MLP disagreement
# (from your previous step)
TEST_WITH_DISAGREE_CSV = r"C:\Users\poula\Desktop\quad redo\Bace 1\outputs\bace1_test_with_disagreement.csv"
...
def bitstring_to_bv(bitstring: str):
    # RDKit ExplicitBitVect from "010101..."
...
def main():
    ...
    # ---- Load your existing test-with-disagreement file (authoritative test set table)
    ...
    # ---- Map fingerprints for those test smiles (merge to get fingerprint column)
    # This ensures the AD computation uses the same test molecules you’ve been analyzing.
    ...
    # ---- Save
    out_csv = os.path.join(OUT_DIR, "bace1_test_with_ad_risk.csv")
    merged.to_csv(out_csv, index=False)

    print("\n✅ AD risk computed (BACE1)")
    print("Saved:", out_csv)
    print("Columns added:")
